...
全球首个将 轻量化低功耗推理 量化算法落地到 FPGA 硬件的加速器。轻量化低功耗推理算法将权重约束为 极低精度，把计算量大幅降低，让每一块 ¥200 级国产 FPGA 都能运行 AI Transformer 推理。
...
- **加速层**：轻量化低功耗推理 16×16 Tile 阵列（三值矩阵乘法 → 加减法，33% 稀疏跳过）
...
| Phase 4（18–24月） | 推理加速IP核授权（RTL IP） | FPGA / SoC 厂商 | 联系获取 |
