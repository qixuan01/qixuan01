...
description: "World's first 轻量化低功耗推理FPGA hardware accelerator · RISC-V + Anlogic FPGA + HarmonyOS LiteOS · 100% domestic · BOM <¥200"
weight: 1
---
...
<div class="edge-page-header">
  ...
  <p>鸿源边缘AI推理加速器</p>
  <p>World's first 轻量化低功耗推理FPGA hardware accelerator · Full-stack self-designed RISC-V SoC</p>
</div>

## Product Highlights

> As of June 2026, no 轻量化低功耗推理-specific FPGA/ASIC accelerators exist in global academic literature or patents. This project is the world's first experimental platform to deploy 轻量化低功耗推理 on FPGA hardware, with a 12–18 month first-mover advantage.

<div class="edge-product-showcase">
  <div class="edge-product-showcase-label">🔴 Hongyuan Edge AI Inference Accelerator · World's First 轻量化低功耗推理FPGA Hardware Accelerator</div>
...
## Core Innovation: 轻量化低功耗推理 lightweight low-power inference

In traditional neural network inference, multipliers consume 70%+ of FPGA area and 60%+ of power. 轻量化低功耗推理 constrains weights to 极低精度, reducing all multiplication to addition/subtraction — enabling full Transformer inference on a 20K LUT low-end FPGA.

This is co-innovation across algorithm, architecture, and circuit layers simultaneously.

| Metric | Traditional FP16 | Hongyuan 轻量化低功耗推理 |
|---|---|---|
| Weight Precision | 16-bit float | ultra-low-bit ternary {-1,0,+1} |
...
|---|---|---|---|
| Step 1 | 16×16 轻量化低功耗推理 Tile | ✅ 256/256 PASS | 8,192 cycles, 1,280 zero weights auto-skipped |
...
<div class="edge-arch-stack">
  ...
  <div class="edge-arch-layer edge-arch-accel">
    ...
    <span class="edge-arch-content">轻量化低功耗推理 16×16 Tile Array — lightweight inference matrix multiply → add/subtract, 33% sparse skip</span>
  </div>
...
- **Invention Patent**: 轻量化低功耗推理FPGA accelerator architecture patent pending
